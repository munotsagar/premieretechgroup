function multiUploader(config){
	...
	multiUploader.prototype._dropFiles = function(e){
		...
		self._uploader(e.dataTransfer.files,0);
	}
	...
	multiUploader.prototype._uploader = function(file,f){
		if(typeof file[f] != undefined && self._validate(file[f].type) > 0){
			...
			var recId = $("#idrec").val();
			var toRec = $("textarea[name='attachment_c']").val();
			
			data.append('file',file[f]);
			data.append('index',ids);
			data.append('record',recId);
			data.append('updateField',toRec);
			...
			$.ajax({
				...
				success:function(rponse){
					...
					var obj = $(".dfiles").get();
							//console.log(rponse)
							var divTxt = "<tr><td class='txtStr'><a href='uploads/"+rponse+"' target='_blank'>"+rponse+"</a></td><td><a class='delCls'><img src='images/trash.png' height='20'></a></tr>";
							$("#tblId").append(divTxt);
							//$("#tblId").append(divTxt);
							//alert(rponse)
							console.log($("textarea[name='attachment_c']").val());
							//alert($("textarea[name='attachment_c']").val());
							if($("textarea[name='attachment_c']").val() == ""){
								$("textarea[name='attachment_c']").val(rponse);
								
							}else{
								$("textarea[name='attachment_c']").val($("textarea[name='attachment_c']").val()+","+	rponse);
							}
							console.log($("textarea[name='attachment_c']").val());
							$(this).remove();
